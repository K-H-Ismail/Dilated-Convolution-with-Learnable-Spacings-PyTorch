...
class _DclsNd(Module):
    ...
    def reset_parameters(self) -> None:
        ...
        for i in range(len(self.kernel_size)): 
            init.uniform_(self.P.select(0,i), a=-1/math.sqrt(self.out_channels * self.kernel_size[0] * self.kernel_size[1])
                                            , b= 1/math.sqrt(self.out_channels * self.kernel_size[0] * self.kernel_size[1])) 
    ...
    def _conv_forward(self, input: Tensor, weight: Tensor, bias: Optional[Tensor], P: Tensor):
        if self.padding_mode != 'zeros':
            return F.conv1d(F.pad(input, self._reversed_padding_repeated_twice, mode=self.padding_mode),
                            SD.SurrogateDilation1d.apply(weight, P, self.dilation), bias, self.stride,
                            _single(0), _single(1), self.groups)
        return F.conv1d(input, SD.SurrogateDilation1d.apply(weight, P, self.dilation), bias, self.stride,
    ...
    * :attr:`dilation` controls the spacing between the kernel points; also
    ...
    def _conv_forward(self, input: Tensor, weight: Tensor, bias: Optional[Tensor], P1: Tensor, P2: Tensor):
        if self.padding_mode != 'zeros':
            return F.conv2d(F.pad(input, self._reversed_padding_repeated_twice, mode=self.padding_mode),
                            SD.SurrogateDilation2d.apply(weight, P1, P2, self.dilation), bias, self.stride,
                            _pair(0), _pair(1), self.groups)
        return F.conv2d(input, SD.SurrogateDilation2d.apply(weight, P1, P2, self.dilation), bias, self.stride,
    ...
    def _conv_forward_swc(self, input: Tensor, weight: Tensor, bias: Optional[Tensor], P1: Tensor, P2: Tensor):
        if self.padding_mode != 'zeros':
            return SW.swc2d.apply(F.pad(input, self._reversed_padding_repeated_twice, mode=self.padding_mode),
                            SD.SurrogateDilation2d.apply(weight, P1, P2, self.dilation), bias, self.stride,
                            _pair(0), _pair(1), self.groups)
        return SW.swc2d.apply(input, SD.SurrogateDilation2d.apply(weight, P1, P2, self.dilation), bias, self.stride,
                        self.padding, _pair(1), self.groups)
    ...
    def forward(self, input: Tensor) -> Tensor:
        if (self.dilation[0] * self.dilation[1] == 1) :
            ...
            nb_chunks = (self.out_channels-1)//self.chunk_size + 1; 

            chunked_P_h = self.P.select(0,0).chunk(nb_chunks,0);
            chunked_P_w = self.P.select(0,1).chunk(nb_chunks,0);

            chunked_weights = self.weight.chunk(nb_chunks,0);
            chunked_biases = self.bias.chunk(nb_chunks,0);            
            for chunk in range(nb_chunks):
                P_h_chunk = chunked_P_h[chunk];
                P_w_chunk = chunked_P_w[chunk];
                weights_chunk = chunked_weights[chunk];
                bias_chunk = chunked_biases[chunk];                

                output_chunk = self._conv_forward(input, weights_chunk, bias_chunk, P_h_chunk, P_w_chunk);
                output = output_chunk if chunk == 0 else torch.cat((output,output_chunk),1)
    ...
    def _conv_forward(self, input: Tensor, weight: Tensor, bias: Optional[Tensor], P1: Tensor, P2: Tensor, P3: Tensor):
        if self.padding_mode != 'zeros':
            return F.conv3d(F.pad(input, self._reversed_padding_repeated_twice, mode=self.padding_mode),
                            SD.SurrogateDilation3d.apply(weight, P1, P2, P3, self.dilation), bias, self.stride,
                            _triple(0), _triple(1), self.groups)
        return F.conv3d(input, SD.SurrogateDilation3d.apply(weight, P1, P2, P3, self.dilation), bias, self.stride,
...
class Dcls2_1d(_DclsN_Md):
    ...
    * :attr:`dilation` controls the spacing between the kernel points; also
    ...
    def _conv_forward(self, input: Tensor, weight: Tensor, bias: Optional[Tensor], P1: Tensor):
        if self.padding_mode != 'zeros':
            return F.conv2d(F.pad(input, self._reversed_padding_repeated_twice, mode=self.padding_mode),
                            SD.SurrogateDilation2_1d.apply(weight, P1, self.dilation), bias, self.stride,
                            _pair(0), _pair(1), self.groups)
        return F.conv2d(input, SD.SurrogateDilation2_1d.apply(weight, P1, self.dilation), bias, self.stride,
                        self.padding, _pair(1), self.groups)
...
class Dcls3_1d(_DclsN_Md):
    ...
    def _conv_forward(self, input: Tensor, weight: Tensor, bias: Optional[Tensor], P1: Tensor):
        if self.padding_mode != 'zeros':
            return F.conv3d(F.pad(input, self._reversed_padding_repeated_twice, mode=self.padding_mode),
                            SD.SurrogateDilation3_1d.apply(weight, P1, self.dilation), bias, self.stride,
                            _triple(0), _triple(1), self.groups)
        return F.conv3d(input, SD.SurrogateDilation3_1d.apply(weight, P1, self.dilation), bias, self.stride,
                        self.padding, _triple(1), self.groups)
...
class Dcls3_2d(_DclsN_Md):
    ...
    def _conv_forward(self, input: Tensor, weight: Tensor, bias: Optional[Tensor], P1: Tensor, P2: Tensor):
        if self.padding_mode != 'zeros':
            return F.conv3d(F.pad(input, self._reversed_padding_repeated_twice, mode=self.padding_mode),
                            SD.SurrogateDilation3_2d.apply(weight, P1, P2, self.dilation), bias, self.stride,
                            _triple(0), _triple(1), self.groups)
        return F.conv3d(input, SD.SurrogateDilation3_2d.apply(weight, P1, P2, self.dilation), bias, self.stride,
                        self.padding, _triple(1), self.groups)
...
class Dcls(_DclsNd):
    ...
    * :attr:`dilation` controls the spacing between the kernel points; also
    ...
    def _conv_forward(self, input: Tensor, weight: Tensor, bias: Optional[Tensor], P1: Tensor, P2: Tensor):
        if self.padding_mode != 'zeros':
            return SD.dcls_conv.aply(F.pad(input, self._reversed_padding_repeated_twice, mode=self.padding_mode),
                            weight, P1, P2, bias, self.stride, _pair(0), self.dilation, self.groups)
        return SD.dcls_conv.apply(input, weight, P1, P2, bias, self.stride, self.padding, self.dilation, self.groups)
    def forward(self, input: Tensor) -> Tensor:
        if (self.dilation[0] * self.dilation[1] == 1) :
            return F.conv2d(input, self.weight, self.bias, self.stride, self.padding, _pair(1), self.groups)
        else:        
