...
void sparse_mm_dense_cusparse_backend_lt(const int & cuda_device_id, const int & m, const int & n, const int & p, float * dA, float * dB, float * dC);
...
void sparse_mm_dense_cusparse_lt(torch::Tensor & A, torch::Tensor & B, torch::Tensor & C)
{   
    // A is sparse, B is dense
    TORCH_CHECK(A.device().is_cuda(), "A must be a CUDA tensor");
    TORCH_CHECK(B.device().is_cuda(), "B must be a CUDA tensor");
    ...
    int n = A.size(1);
    int p = B.size(1);
    sparse_mm_dense_cusparse_backend_lt(B.get_device(), m, n, p, A.data_ptr<float>(), B.data_ptr<float>(), C.data_ptr<float>());

}

PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
    m.def("sparse_mm_dense_cusparse", &sparse_mm_dense_cusparse);
    m.def("sparse_mm_dense_cusparse_lt", &sparse_mm_dense_cusparse_lt);
