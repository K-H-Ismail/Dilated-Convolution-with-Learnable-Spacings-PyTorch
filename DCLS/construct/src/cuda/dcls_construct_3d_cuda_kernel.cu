...
__global__ void interpolation_kernel(
    const int n,
    torch::PackedTensorAccessor32<scalar_t,3,torch::RestrictPtrTraits> P_d,
    ...
    torch::PackedTensorAccessor32<scalar_t,3,torch::RestrictPtrTraits> W5,
    torch::PackedTensorAccessor32<scalar_t,3,torch::RestrictPtrTraits> W6,
    torch::PackedTensorAccessor32<scalar_t,3,torch::RestrictPtrTraits> W7,
    torch::PackedTensorAccessor32<scalar_t,3,torch::RestrictPtrTraits> W8,
  ...
  CUDA_KERNEL_LOOP(index, n) {
    ...
    int p_d = P_d[channel_out][channel_in][k];
    ...
    {
        atomicAdd(&interpolated_weight[channel_out][channel_in][p_d][p_h][p_w], W1[channel_out][channel_in][k]);
        if(p_d_next < depth_out)
            atomicAdd(&interpolated_weight[channel_out][channel_in][p_d_next][p_h][p_w], W2[channel_out][channel_in][k]);
        if(p_h_next < height_out)
            atomicAdd(&interpolated_weight[channel_out][channel_in][p_d][p_h_next][p_w], W3[channel_out][channel_in][k]);
        if(p_d_next < depth_out & p_h_next < height_out)
            atomicAdd(&interpolated_weight[channel_out][channel_in][p_d_next][p_h_next][p_w], W4[channel_out][channel_in][k]);
        if(p_w_next < width_out)
            atomicAdd(&interpolated_weight[channel_out][channel_in][p_d][p_h][p_w_next], W5[channel_out][channel_in][k]);
        if(p_d_next < depth_out & p_w_next < width_out)
            atomicAdd(&interpolated_weight[channel_out][channel_in][p_d_next][p_h][p_w_next], W6[channel_out][channel_in][k]);
        if(p_h_next < height_out & p_w_next < width_out)
            atomicAdd(&interpolated_weight[channel_out][channel_in][p_d][p_h_next][p_w_next], W7[channel_out][channel_in][k]);
        if(p_d_next < depth_out & p_h_next < height_out & p_w_next < width_out)
            atomicAdd(&interpolated_weight[channel_out][channel_in][p_d_next][p_h_next][p_w_next], W8[channel_out][channel_in][k]);
    }
  }
}
  ...
  CUDA_KERNEL_LOOP(index, n) {
    ...
    if(p_d >= 0 & p_d < depth_out & p_h >= 0 & p_h < height_out & p_w >= 0 & p_w < width_out)
    {
        atomicAdd(&interpolated_weight[channel_out][channel_in][k],
          grad_output[channel_out][channel_in][p_d][p_h][p_w] * W1[channel_out][channel_in][k]);
        if(p_d_next < depth_out)
            atomicAdd(&interpolated_weight[channel_out][channel_in][k],
              grad_output[channel_out][channel_in][p_d_next][p_h][p_w] * W2[channel_out][channel_in][k]);
        if(p_h_next < height_out)
            atomicAdd(&interpolated_weight[channel_out][channel_in][k],
              grad_output[channel_out][channel_in][p_d][p_h_next][p_w] * W3[channel_out][channel_in][k]);
        if(p_d_next < depth_out & p_h_next < height_out)
            atomicAdd(&interpolated_weight[channel_out][channel_in][k],
              grad_output[channel_out][channel_in][p_d_next][p_h_next][p_w] * W4[channel_out][channel_in][k]);
        if(p_w_next < width_out)
            atomicAdd(&interpolated_weight[channel_out][channel_in][k],
              grad_output[channel_out][channel_in][p_d][p_h][p_w_next] * W5[channel_out][channel_in][k]);
        if(p_d_next < depth_out & p_w_next < width_out)
            atomicAdd(&interpolated_weight[channel_out][channel_in][k],
              grad_output[channel_out][channel_in][p_d_next][p_h][p_w_next] * W6[channel_out][channel_in][k]);
        if(p_h_next < height_out & p_w_next < width_out)
            atomicAdd(&interpolated_weight[channel_out][channel_in][k],
              grad_output[channel_out][channel_in][p_d][p_h_next][p_w_next] * W7[channel_out][channel_in][k]);
        if(p_d_next < depth_out & p_h_next < height_out & p_w_next < width_out)
            atomicAdd(&interpolated_weight[channel_out][channel_in][k],
              grad_output[channel_out][channel_in][p_d_next][p_h_next][p_w_next] * W8[channel_out][channel_in][k]);
    }
  }
}
...
torch::Tensor  dcls_construct_3d_cuda_forward(
    ...
    torch::Tensor P2,
    torch::Tensor P3,
    const int dilated_kernel_size_d,
    const int dilated_kernel_size_h,
    const int dilated_kernel_size_w,
    const float scaling
    ) {
    ...
    // Bounds for Pd, Ph and Pw
    const int half_range_d = dilated_kernel_size_d / 2;
    const int half_range_h = dilated_kernel_size_h / 2;
    const int half_range_w = dilated_kernel_size_w / 2;

    // Preform scaling
    auto scaled_P1 = P1*scaling /*+ at::arange(-half_range_bot_d + dilation_d/2,half_range_bot_d + 1e-7,dilation_d, weight.options())
                            .repeat({kernel_h,kernel_w,1})
                            .permute({2,0,1})
                            .repeat({channels_out,channels_in,1,1,1})*/;
    auto scaled_P2 = P2*scaling /*+ at::arange(-half_range_bot_h + dilation_h/2,half_range_bot_h + 1e-7,dilation_h, weight.options())
                            .repeat({kernel_d,kernel_w,1})
                            .permute({0,2,1})
                            .repeat({channels_out,channels_in,1,1,1})*/;
    auto scaled_P3 = P3*scaling /*+ at::arange(-half_range_bot_w + dilation_w/2,half_range_bot_w + 1e-7,dilation_w, weight.options())
    ...
    auto P_h = scaled_P2 + half_range_h;
    auto P_w = scaled_P3 + half_range_w;
    ...
    P_d = P_d.clamp(0, dilated_kernel_size_d - 1);
    P_h = P_h.clamp(0, dilated_kernel_size_h - 1);
    P_w = P_w.clamp(0, dilated_kernel_size_w - 1);

    // Calculate rests for interpolation
    auto rest_d = (scaled_P1 + half_range_d).clamp(0, dilated_kernel_size_d - 1) - P_d;
    auto rest_h = (scaled_P2 + half_range_h).clamp(0, dilated_kernel_size_h - 1) - P_h;
    auto rest_w = (scaled_P3 + half_range_w).clamp(0, dilated_kernel_size_w - 1) - P_w;
    ...
    auto output = torch::zeros({channels_out, channels_in, dilated_kernel_size_d, dilated_kernel_size_h, dilated_kernel_size_w}, weight.options());
    ...
    AT_DISPATCH_FLOATING_TYPES(weight.type(), "dcls_construct_3d_forward_cuda", [&] {
        ...
        interpolation_kernel<scalar_t><<<GET_BLOCKS(num_kernels), 1024, 0, at::cuda::getCurrentCUDAStream()>>>(
                                     num_kernels,
                                     P_d.packed_accessor32<scalar_t,3,torch::RestrictPtrTraits>(),
                                     ...
                                     W5.packed_accessor32<scalar_t,3,torch::RestrictPtrTraits>(),
                                     W6.packed_accessor32<scalar_t,3,torch::RestrictPtrTraits>(),
                                     W7.packed_accessor32<scalar_t,3,torch::RestrictPtrTraits>(),
                                     W8.packed_accessor32<scalar_t,3,torch::RestrictPtrTraits>(),
...
std::vector<torch::Tensor> dcls_construct_3d_cuda_backward(
    ...
    auto mask_d = rest_d.gt(0) * rest_d.lt(dilated_kernel_size_d-1);
    rest_d = rest_d.clamp(0,dilated_kernel_size_d-1) - P_d;
    auto rest_h = scaled_P2 + half_range_h;
    auto mask_h = rest_h.gt(0) * rest_h.lt(dilated_kernel_size_h-1);
    rest_h = rest_h.clamp(0,dilated_kernel_size_h-1) - P_h;
    auto rest_w = scaled_P3 + half_range_w;
    ...
    auto W1_Pd = - weight * mask_d + rhW + rwW - rhwW;
    auto W2_Pd = - W1_Pd;
    auto W3_Pd = - rhW + rhwW;
    auto W4_Pd = - W3_Pd;
    auto W5_Pd = - rwW + rhwW;
    ...
    auto W1_Ph = - weight * mask_h + rdW + rwW - rdwW;
    auto W2_Ph = - rdW + rdwW;
    auto W3_Ph = - W1_Ph;
    auto W4_Ph = - W2_Ph;
    auto W5_Ph = - rwW + rdwW;
    ...
    auto W1_Pw = - weight * mask_w + rdW + rhW - rdhW;
    auto W2_Pw = - rdW + rdhW;
    auto W3_Pw = - rhW + rdhW;
    auto W4_Pw = - rdhW;
    auto W5_Pw = - W1_Pw;
    ...
    AT_DISPATCH_FLOATING_TYPES(weight.type(), "dcls_construct_3d_backward_cuda", [&] {
        ...
        interpolation_grad_kernel<scalar_t><<<GET_BLOCKS(num_kernels), 1024, 0, at::cuda::getCurrentCUDAStream()>>>(
                                     ...
                                     P_h.packed_accessor32<scalar_t,3,torch::RestrictPtrTraits>(),
                                     P_w.packed_accessor32<scalar_t,3,torch::RestrictPtrTraits>(),
                                     W1_Pd.packed_accessor32<scalar_t,3,torch::RestrictPtrTraits>(),
                                     W2_Pd.packed_accessor32<scalar_t,3,torch::RestrictPtrTraits>(),
                                     W3_Pd.packed_accessor32<scalar_t,3,torch::RestrictPtrTraits>(),
                                     W4_Pd.packed_accessor32<scalar_t,3,torch::RestrictPtrTraits>(),
                                     W5_Pd.packed_accessor32<scalar_t,3,torch::RestrictPtrTraits>(),
                                     W6_Pd.packed_accessor32<scalar_t,3,torch::RestrictPtrTraits>(),
                                     W7_Pd.packed_accessor32<scalar_t,3,torch::RestrictPtrTraits>(),
                                     W8_Pd.packed_accessor32<scalar_t,3,torch::RestrictPtrTraits>(),
                                     channels_out, channels_in,
                                     kernel,
                                     dilated_kernel_size_d,
                                     dilated_kernel_size_h,
        ...
        interpolation_grad_kernel<scalar_t><<<GET_BLOCKS(num_kernels), 1024, 0, at::cuda::getCurrentCUDAStream()>>>(
                                     ...
                                     W5_Ph.packed_accessor32<scalar_t,3,torch::RestrictPtrTraits>(),
                                     W6_Ph.packed_accessor32<scalar_t,3,torch::RestrictPtrTraits>(),
                                     W7_Ph.packed_accessor32<scalar_t,3,torch::RestrictPtrTraits>(),
                                     W8_Ph.packed_accessor32<scalar_t,3,torch::RestrictPtrTraits>(),
                                     channels_out, channels_in,
                                     kernel,
                                     dilated_kernel_size_d,
                                     dilated_kernel_size_h,
        ...
        interpolation_grad_kernel<scalar_t><<<GET_BLOCKS(num_kernels), 1024, 0, at::cuda::getCurrentCUDAStream()>>>(
                                     ...
                                     W5_Pw.packed_accessor32<scalar_t,3,torch::RestrictPtrTraits>(),
                                     W6_Pw.packed_accessor32<scalar_t,3,torch::RestrictPtrTraits>(),
                                     W7_Pw.packed_accessor32<scalar_t,3,torch::RestrictPtrTraits>(),
                                     W8_Pw.packed_accessor32<scalar_t,3,torch::RestrictPtrTraits>(),
                                     channels_out, channels_in,
                                     kernel,
                                     dilated_kernel_size_d,
                                     dilated_kernel_size_h,
                                     dilated_kernel_size_w,
                                     grad_P3.packed_accessor32<scalar_t,3,torch::RestrictPtrTraits>());
