...
__global__ void interpolation_grad_kernel(
    ...
    torch::PackedTensorAccessor32<scalar_t,4,torch::RestrictPtrTraits> W3, 
    torch::PackedTensorAccessor32<scalar_t,4,torch::RestrictPtrTraits> W4,    
    torch::PackedTensorAccessor32<scalar_t,4,torch::RestrictPtrTraits> W1_Ph, 
    torch::PackedTensorAccessor32<scalar_t,4,torch::RestrictPtrTraits> W2_Ph,
    torch::PackedTensorAccessor32<scalar_t,4,torch::RestrictPtrTraits> W3_Ph, 
    torch::PackedTensorAccessor32<scalar_t,4,torch::RestrictPtrTraits> W4_Ph,
    torch::PackedTensorAccessor32<scalar_t,4,torch::RestrictPtrTraits> W1_Pw, 
    torch::PackedTensorAccessor32<scalar_t,4,torch::RestrictPtrTraits> W2_Pw,
    torch::PackedTensorAccessor32<scalar_t,4,torch::RestrictPtrTraits> W3_Pw, 
    torch::PackedTensorAccessor32<scalar_t,4,torch::RestrictPtrTraits> W4_Pw,    
    const int ch_out, const int ch_in,
    const int kernel_h, const int kernel_w,
    const int height_out, const int width_out,     
    torch::PackedTensorAccessor32<scalar_t,4,torch::RestrictPtrTraits>  interpolated_weight,
    torch::PackedTensorAccessor32<scalar_t,4,torch::RestrictPtrTraits>  interpolated_P1,
    torch::PackedTensorAccessor32<scalar_t,4,torch::RestrictPtrTraits>  interpolated_P2) {
  CUDA_KERNEL_LOOP(index, n) {
    int w_out = index % kernel_w;
    int h_out = (index / kernel_w) % kernel_h;
    int channel_in = (index / kernel_h / kernel_w) % ch_in;
    ...
    interpolated_P1[channel_out][channel_in][h_out][w_out]= g1 * w * W1_Ph[channel_out][channel_in][h_out][w_out]
                                                                 + g2 * w * W2_Ph[channel_out][channel_in][h_out][w_out]
                                                                 + g3 * w * W3_Ph[channel_out][channel_in][h_out][w_out]
                                                                 + g4 * w * W4_Ph[channel_out][channel_in][h_out][w_out];
      
    interpolated_P2[channel_out][channel_in][h_out][w_out]= g1 * w * W1_Pw[channel_out][channel_in][h_out][w_out]
                                                                 + g2 * w * W2_Pw[channel_out][channel_in][h_out][w_out]
                                                                 + g3 * w * W3_Pw[channel_out][channel_in][h_out][w_out]
                                                                 + g4 * w * W4_Pw[channel_out][channel_in][h_out][w_out];      
